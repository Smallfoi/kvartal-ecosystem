...
String oemBackgroundHint(String manufacturer) {
  ...
  if (m.contains('infinix') ||
    ...
    return 'Настройки → Приложения → ЛИГА: включи «Автозапуск», сними ограничения '
  ...
  if (m.contains('xiaomi') || m.contains('redmi') || m.contains('poco')) {
    return 'Настройки → Приложения → ЛИГА: «Автозапуск» — вкл; «Контроль активности» — '
        'без ограничений; «Экономия батареи» — без ограничений.';
  }
  if (m.contains('huawei') || m.contains('honor')) {
    return 'Настройки → Батарея → Запуск приложений → ЛИГА: ручное управление, '
  ...
  if (m.contains('oppo') ||
      ...
      m.contains('vivo') ||
      m.contains('iqoo')) {
    return 'Настройки → Батарея → ЛИГА: разреши фоновую работу и автозапуск; '
        'сними «Умную экономию» для приложения.';
  }
  if (m.contains('samsung')) {
    return 'Настройки → Батарея → Ограничения в фоне: убери ЛИГА из спящих приложений; '
        'в приложении выбери «Без ограничений».';
  }
  return 'В настройках батареи разреши ЛИГА работать в фоне без ограничений '
