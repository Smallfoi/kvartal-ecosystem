...
/// анимаций МАТА): тёмная бренд-панель проезжает через весь экран по кривой
...
/// эталона (±200% относительно себя). Смена контента/навигация происходит в
/// момент полного покрытия экрана.
///
/// Живёт в корневом Overlay, поэтому переживает смену роутов. Если [onCovered]
/// возвращает Future, шторка держит покрытие до его завершения (ожидание
/// сервера — как «докрутка» в OTP V5). При reduced motion шторки нет —
/// сразу вызывается [onCovered].
...
class _BrandSweepState extends State<_BrandSweep>
  ...
  Future<void> _run() async {
    // Въезд до полного покрытия (панель проходит свою ширину — как в эталоне
    // 600 мс на ширину; здесь полширины пути за 420 мс — тот же характер).
    await _c.animateTo(
      0.5,
                ...
                child: Container(
                  decoration: const BoxDecoration(
                    gradient: LinearGradient(
                      ...
                      colors: [Color(0xFF10151D), AppColors.bgDark],
                    ),
                  ),
