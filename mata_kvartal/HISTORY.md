...
- PRD.md скопирован из `mata_kvartal` → `kvartal_app` (объединение папок)
...
- Зафиксировано, что старая папка `C:\Users\crypt\mata_kvartal` больше не является рабочей директорией.
...
### Сделано
- Найден актуальный путь после переименования корневой папки: `D:\MyProjectsCLAUDE\mata_kvartal`.
- Собран debug APK из пути без кириллицы.
- APK установлен на телефон `143332557B103525`.
- Настроен `adb reverse tcp:3000 tcp:3000`.
- Запущен локальный Go backend из `D:\MyProjectsCLAUDE\mata_kvartal\backend\kvartal_server.exe`.
