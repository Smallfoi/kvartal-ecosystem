...
Второй фактор обязателен ДЛЯ ВСЕХ, включая владельца (D-68). Раньше владелец
был исключён: считалось, что он подключит себе код командой на сервере и это
его личное дело. На практике вышло наоборот — на проде код оказался у сотрудника
и не оказался у владельца, то есть самая ценная учётная запись держалась на
одном пароле. Теперь без привязанного устройства в админку не попадает никто.
