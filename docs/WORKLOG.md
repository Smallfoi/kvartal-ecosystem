...
## 2026-06-24 — Claude — Подготовка к продакшну: 6 PR (#117–#122)
...
**Решения:** см. правки `PRODUCTION_HARDENING.md` (галочки по fail-fast/баланс/бан/удаление/прод-конфиг).
**Состояние:** ~40 тестов зелёные под CI; бэкенд готов к деплою одной командой, как только у владельца будут домены/секреты/хостинг.
**Дальше:** 🔑 владельцу — SMS-провайдер, Yandex Cloud, домены+TLS, Sentry/AppMetrica, RuStore/VK push, юр-тексты+РКН+возраст, смена пароля админки. Кодом: продолжение S-04 (риск-скоринг, mock-GPS, мульти-аккаунт), clubs/leaderboard/shoes-тесты.
