...
## 2026-06-21 — Claude — Погода: реальная t° (MET Norway) + рабочий морозный коэффициент (D-20)
**Сделано:** довёл погоду до конца — раньше был только хардкод-чип `−24°C ×1.4` (предыдущая сессия), без логики.
- `features/weather/data/weather_provider.dart`: тянет текущую t° по координатам с **MET Norway** (api.met.no — бесплатно, без ключа, коммерция ок, см. D-20; User-Agent + кэш 20 мин). Открытый источник, как карта.
- `frostMultiplier(t°)`: ниже −10°C бонус, потолок ×2.0. Чип на карте теперь показывает **реальную t°** и множитель (бейдж только когда есть мороз).
- **Морозный коэффициент стал реальным:** в `_awardLoyaltyPoints` очки за км умножаются на множитель (`км×10×mult`) — раньше ×1.4 был чисто декоративным.
- **Проверено на устройстве:** чип показал **17°C** (реальное лето Якутска, не фейковые −24), бейдж скрыт (тепло → ×1.0). Юнит-тесты `frostMultiplier` (−10→1.0, −20→1.2, −30→1.4, потолок 2.0) зелёные; analyze + flutter test ок.
**Дальше:** серверная валидация множителя (античит, S-04); атрибуцию MET в «О приложении».
