...
Xiaomi; у Garmin поддержка появляется. Это значит: человек включает у себя в
...
1. **Health Connect (Android)** — теперь это первый шаг, а не второй. Системное хранилище
   здоровья на Android: заявка не нужна, а тренировки приходят с часов COROS, Samsung,
   Amazfit, Polar и прочих — со всех, чьи приложения туда пишут. Человеку достаточно
   разрешить чтение.
