...
- **Клиентский JS сайта (`content.js`/`editor.js`) НЕ покрыт CI** (CI = Flutter×2 + Django; статик-сайт вне
...
- **Решение — один скрипт `scripts/start-dev.ps1`** (Docker+migrate+adb reverse+сайт
  ...
  <порт> <папка>`. Превью приложения (5578) требует `sport_store/build/web` — после
  правок sport_store пересобрать: `flutter build web --pwa-strategy=none --dart-define=PREVIEW=1`.
...
- **Перед `git add`/commit — проверять `git status`:** в рабочем дереве могут лежать ЧУЖИЕ незакоммиченные изменения (WIP параллельного чата) или застейдженные удаления. Раз так в коммит просочилось удаление `sport_store/IDEAS.md` (#182). Добавлять ТОЛЬКО свои файлы по явным путям и сверять `git status -s` до коммита.
...
## Сайт STAW — профиль / уровни (РЕГРЕСС, владелец флагал дважды)
