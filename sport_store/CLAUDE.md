...
> ⚠️ **Актуально:** магазин **подключён к общему Django-бэкенду** (auth, loyalty, catalog, orders,
> notifications, account: удаление+приватность) — это **не только mock**. Разделы «Известные
> ограничения» и «Рекомендованный этап» ниже **частично исторические** (писались на mock-этапе).
> Источник правды по состоянию — `docs/` (WORKLOG, DECISIONS, ECOSYSTEM_API) и память
> `project-sport-store-changelog`.
