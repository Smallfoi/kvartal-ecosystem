...
/// Тип «живой» мини-анимации иконки инструмента.
enum _ToolAnim { sweep, beat, slide, swing, spin }

/// Раздел «Инструменты» на экране «Бег»: горизонтальная лента ярлыков к
/// офлайн-инструментам. Иконки анимированы («живые»), под каждым — короткое
/// описание, по бокам — стрелки-подсказки для листания (тап листает ленту).
...
class _ToolsSectionState extends State<_ToolsSection> {
  ...
  void _nudge(int dir) {
    ...
    final target = (_sc.offset + dir * 190).clamp(
            ...
            children: [
              ...
              Expanded(
                child: ListView.separated(
                  ...
                  itemBuilder: (_, i) => _ToolShortcut(entry: _tools[i]),
                ),
              ),
              _ScrollArrow(
...
/// Боковая стрелка-подсказка «есть ещё карточки»; по тапу листает ленту.
...
class _ToolShortcut extends StatelessWidget {
  ...
  entry;
  const _ToolShortcut({required this.entry});

  /// Иконка с «живой» зацикленной анимацией (у каждого инструмента своя).
  Widget _animatedIcon() {
    final base = Icon(entry.icon, color: entry.color, size: 20);
    switch (entry.anim) {
      case _ToolAnim.beat:
        return base
      ...
      case _ToolAnim.sweep:
        // «Стрелка спидометра» — заметно покачивается влево-вправо.
        return base
            .animate(onPlay: (c) => c.repeat(reverse: true))
            .rotate(
              begin: -0.075,
